...
type Item = {
  ...
  focus: string[];
  impact: string[];
  phone: 'green' | 'sports' | 'pmb' | 'tempo' | 'anugerah' | 'runway' | 'resume' | 'home';
  url: string;
};
...
const work: Item[] = [
  ...
  { company: 'Anugerah Printer', title: 'Company Profile Website', role: 'Frontend Developer', date: 'Printing business website', description: 'Static company profile website for a Jakarta printing business with service pages, portfolio filtering, WhatsApp quote intake, and location details.', focus: ['Vanilla HTML/CSS/JavaScript', 'Tailwind CDN', 'Static MPA architecture', 'WhatsApp quote flow', 'Portfolio gallery + lightbox', 'Leaflet map', 'Local SEO metadata'], impact: ['Built a clearer service-first website for printing customers', 'Added portfolio filtering to showcase different print categories', 'Improved inquiry flow with direct WhatsApp quote requests'], phone: 'anugerah', url: 'https://anugerahprinter.com/' },
...
const workId: Item[] = [
  ...
  { ...work[4], date: 'Website bisnis percetakan', description: 'Website company profile statis untuk bisnis percetakan Jakarta dengan halaman layanan, filter portfolio, quote WhatsApp, dan detail lokasi.', focus: ['Vanilla HTML/CSS/JavaScript', 'Tailwind CDN', 'Arsitektur MPA statis', 'Alur quote WhatsApp', 'Galeri portfolio + lightbox', 'Leaflet map', 'Metadata SEO lokal'], impact: ['Membuat website layanan percetakan yang lebih jelas untuk calon pelanggan', 'Menambahkan filter portfolio untuk menampilkan kategori hasil cetak', 'Mempermudah alur inquiry lewat request quote langsung ke WhatsApp'] },
  { ...work[5], description: 'Perkakas command-line yang memberi tombol undo instan saat agen coding AI merusak codebase — rollback bertingkat, backup otomatis, dan audit log sebelum aksi destruktif.', focus: ['Node 18 + TypeScript', 'tsup build', 'Clack prompts', 'cross-spawn'], impact: ['Merancang UX CLI untuk skenario yang kritis terhadap keamanan', 'Meneliti deteksi perubahan berisiko seperti dependency shift dan schema drift sebelum menimbulkan kerusakan'] },
];
...
function ContributionGraph({ lang }: { lang: Lang }) {
  ...
  const statusMessages = {
    en: {
      high: [
        "Deep in the zone, do not disturb",
        "Third coffee, first bug fixed",
        "Shipped before lunch, feeling unstoppable",
        "Refactoring at 3am and loving it",
        "Chasing a bug that doesn't exist",
        "Git log looking clean today",
        "Cursor's been moving nonstop for an hour",
        "Just pushed, fingers crossed",
        "On a roll, don't jinx it",
        "Whole day disappeared into this feature",
        "Four PRs merged before noon",
        "Finally cracked that race condition",
        "Terminal's been open since 6am",
        "Flow state activated, snacks nearby",
        "Tests green, mood green too",
        "Skipped breakfast for this commit streak",
        "Headphones on, world off",
        "Found the root cause, feels amazing",
        "Two features down, one to go",
        "This is the good kind of tired"
      ],
      medium: [
        "Few commits in, few more coffee breaks",
        "Making progress, slower than I'd like",
        "Half focused, half distracted by Slack",
        "Steady pace, nothing broken yet",
        "Chipping away at the backlog",
        "Some wins, some rabbit holes",
        "Bouncing between two tickets",
        "Decent morning, unclear afternoon",
        "Small wins add up eventually",
        "Distracted but still shipping",
        "One eye on code, one on lunch",
        "Getting there, just not fast",
        "Mixed bag of progress today",
        "In between deep work and small talk"
      ],
      low: [
        "Staring at the screen, nothing's happening",
        "Still on my first coffee, be patient",
        "Stuck in meetings all day",
        "Brain's fried, code can wait",
        "Haven't opened the editor yet, don't judge",
        "Taking the day slow on purpose",
        "Reading docs instead of writing code",
        "Everything's on pause right now",
        "Not proud of today's commit count",
        "Waiting for motivation to show up",
        "Browser has 30 tabs, none of them useful",
        "Today's a thinking day, not a coding day",
        "Existing, barely typing",
        "Rebooting my brain, please hold"
      ]
    },
    id: {
      high: [
        "Lagi fokus banget, jangan diganggu",
        "Kopi ketiga, bug pertama kelar",
        "Udah ngeship sebelum makan siang",
        "Refactor jam 3 pagi dan enjoy aja",
        "Ngoding tanpa nengok WA sejam lebih",
        "Git log rapi hari ini, jarang-jarang",
        "Baru push, semoga CI-nya lolos",
        "Lagi kenceng, jangan disemprot dulu",
        "Sehari full kelarin satu fitur doang",
        "Empat PR kelar sebelum siang",
        "Akhirnya nemu race condition-nya",
        "Terminal udah nyala dari jam 6",
        "Lagi flow, sambil ngemil",
        "Test hijau semua, mood juga ikut hijau",
        "Skip sarapan demi ngejar streak commit",
        "Headset nyala, dunia luar off dulu",
        "Nemu root cause-nya, lega banget",
        "Dua fitur kelar, satu lagi tinggal dikit",
        "Capek yang enak nih rasanya",
        "Lupa waktu gara-gara asik ngoding"
      ],
      medium: [
        "Beberapa commit, beberapa kali rehat kopi juga",
        "Progress jalan, tapi nggak secepet yang dibayangin",
        "Setengah fokus, setengah keganggu chat kerjaan",
        "Santai tapi jalan, belum ada yang rusak",
        "Nyicil backlog pelan-pelan",
        "Ada yang kelar, ada yang malah muter-muter debug",
        "Gantian antara dua tiket",
        "Pagi lumayan, siang belum jelas",
        "Progress kecil tapi lama-lama numpuk",
        "Keganggu tapi tetep jalan",
        "Sambil mikir makan siang, sambil ngoding",
        "Jalan sih, tapi nggak ngebut",
        "Hari ini campur-campur hasilnya",
        "Antara mode serius sama mode ngobrol"
      ],
      low: [
        "Mandangin layar doang, belum ada progress",
        "Kopi pertama belum abis, sabar ya",
        "Kejebak meeting dari pagi",
        "Otak udah lelet, ngoding nanti aja",
        "Belum buka editor, jangan judge dulu",
        "Emang lagi santai-santain hari ini",
        "Baca dokumentasi doang, belum nulis kode",
        "Semua lagi dipause dulu",
        "Nggak pede sama jumlah commit hari ini",
        "Nunggu mood dateng, belum nongol",
        "Tab browser 30, nggak ada yang guna",
        "Hari ini mikir doang, belum ngoding",
        "Ada tapi cuma numpang lewat",
        "Otak lagi restart, tunggu bentar"
      ]
    }
  };
  ...
  const getQuote = (level: number) => {
    let pool;
    if (total >= 150) pool = statusMessages[lang].high;
    else if (total >= 75) pool = statusMessages[lang].medium;
    else pool = statusMessages[lang].low;
...
function WebPreview({ mode }: { mode: Item['phone'] }) {
  ...
  if (mode === 'anugerah') return <SequencedVideo files={['/anugerah.webm']} />;
